...
from garpix_order.models import OrderItem, CorrespondenceItem, CorrespondenceOrderItem
...
class ProductSerializer(serializers.ModelSerializer):
    ...
    count_items_order = serializers.SerializerMethodField()
    #sku_comment
    items_order = serializers.SerializerMethodField()
    url = serializers.SerializerMethodField()
    comment = serializers.SerializerMethodField()
    chat = serializers.SerializerMethodField()
    count_chat = serializers.SerializerMethodField()
    ...
    def get_count(self, obj):
        return OrderItem.objects.filter(order=obj.order).count()
        #return OrderItem.objects.filter(product=obj.product, status=obj.status).count()
        #obj.order.status
        #return OrderItem.objects.filter(product=obj.product).count()

    def get_count_orders(self, obj):
        product_sku = ProductSku.objects.get(id=obj.product.id)
        return OrderItem.objects.filter(product=product_sku, status=obj.status).count()

    def get_count_items_order(self, obj):
        product_sku = ProductSku.objects.get(id=obj.product.id)
        return OrderItem.objects.filter(product=product_sku, status=obj.status, order=obj.order).count()

    def get_url(self, obj):
        return settings.SITE_URL + f'/admin/purchase/purchaseproduct/{obj.id}/change/'

    def get_items_order(self, obj):
        product_sku = ProductSku.objects.get(id=obj.product.id)
        return OrderItem.objects.filter(product=product_sku, status=obj.status)
    ...
    def get_chat(self, obj):
        chat = obj.correspondence_messages.all().order_by('-created_at')
        return chat

    def get_count_chat(self, obj):
        chat = obj.correspondence_messages.all().order_by('-created_at')
        if len(chat) > 0:
            return True
        else:
    ...
    class Meta:
        ...
        fields = [
            ...
            'message', 'order_number', 'order_id', 'count', 'url', 'count_orders', 'items_order', 'count_items_order', 'comment', 'chat', 'count_chat'
        ]
...
class ProductCommentSerializer(serializers.ModelSerializer):
    ...
    def get_photos(self, obj):
        return PhotoSerializer(obj.order_item_comment_photos.all(), many=True).data

    def get_videos(self, obj):
        try:
            video = VideoSerializer(obj.cart_item_comment_videos.all(), many=True).data
        except:
            video = ''
        return video
...
class CorrespondenceOrderItemSerializer(serializers.ModelSerializer):
    ...
    def get_photos(self, obj):
        return PhotoSerializer(obj.correspondence_order_item_message_images.all(), many=True).data



    class Meta:
        model = CorrespondenceOrderItem
        fields = ['author', 'created_at', 'message', 'photos']
